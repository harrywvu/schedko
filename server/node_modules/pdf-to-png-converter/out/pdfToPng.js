 ...
 */
async function processAndSavePage(pdfDocument, pageName, pageNumber, pageViewportScale, shouldReturnContent, returnMetadataOnly, resolvedOutputFolder, realOutputFolder, returnPageContent) {
...
async function pdfToPng(pdfFile, props) {
    ...
    if (typeof pageViewportScale !== 'number' ||
        !Number.isFinite(pageViewportScale) ||
        pageViewportScale <= 0 ||
        pageViewportScale > const_1.MAX_VIEWPORT_SCALE) {
        throw new Error(`viewportScale must be a finite number greater than 0 and at most ${const_1.MAX_VIEWPORT_SCALE}, received: ${pageViewportScale}`);
    }
    ...
    if (props?.processPagesInParallel === true) {
        const concurrencyLimitEarly = props.concurrencyLimit ?? const_1.PDF_TO_PNG_OPTIONS_DEFAULTS.concurrencyLimit;
    ...
    const resolvedOutputFolder = props?.outputFolder !== undefined && !returnMetadataOnly ? (0, node_path_1.join)(process.cwd(), props.outputFolder) : undefined;
    ...
    try {
        const defaultMask = typeof pdfFile === 'string' ? (0, node_path_1.parse)(pdfFile).name : const_1.PDF_TO_PNG_OPTIONS_DEFAULTS.outputFileMask;
        ...
        const shouldReturnContent = returnMetadataOnly ? false : props?.outputFolder ? true : (props?.returnPageContent ?? true);
        if (props?.processPagesInParallel === true) {
            ...
            for (let i = 0; i < validPagesToProcess.length; i += concurrencyLimit) {
                ...
                const batchResults = await Promise.all(batch.map((pageNumber) => processAndSavePage(pdfDocument, resolvePageName(pageNumber, defaultMask, props?.outputFileMaskFunc), pageNumber, pageViewportScale, shouldReturnContent, returnMetadataOnly, resolvedOutputFolder, realOutputFolder, props?.returnPageContent)));
                pngPageOutputs.push(...batchResults);
            }
        }
        else {
            for (const pageNumber of validPagesToProcess) {
                pngPageOutputs.push(await processAndSavePage(pdfDocument, resolvePageName(pageNumber, defaultMask, props?.outputFileMaskFunc), pageNumber, pageViewportScale, shouldReturnContent, returnMetadataOnly, resolvedOutputFolder, realOutputFolder, props?.returnPageContent));
            }
        }
    }
    finally {
...
async function getPdfDocument(pdfFileBuffer, props) {
    ...
    const data = pdfFileBuffer instanceof Uint8Array ? pdfFileBuffer : new Uint8Array(pdfFileBuffer);
...
async function renderPdfPage(pdf, pageName, pageNumber, pageViewportScale, returnPageContent) {
    ...
    if (viewport.width * viewport.height > const_1.MAX_CANVAS_PIXELS) {
        page.cleanup();
        throw new Error(`Canvas ${Math.round(viewport.width)}×${Math.round(viewport.height)} px exceeds the ${const_1.MAX_CANVAS_PIXELS.toLocaleString()} pixel limit. Reduce viewportScale.`);
    }
 ...
 */
function isEscapingRelativePath(rel) {
 ...
 */
async function savePNGfile(pngPageOutput, resolvedOutputFolder, realOutputFolder) {
    ...
    if ((0, node_path_1.isAbsolute)(pngPageOutput.name)) {
    ...
    const resolvedFilePath = (0, node_path_1.join)(resolvedOutputFolder, pngPageOutput.name);
    ...
    if (isEscapingRelativePath((0, node_path_1.relative)(resolvedOutputFolder, resolvedFilePath))) {
        throw new Error(`Output file name escapes the output folder: ${pngPageOutput.name}`);
    }
    ...
    const realFileDir = await node_fs_1.promises.realpath((0, node_path_1.dirname)(resolvedFilePath));
    if (isEscapingRelativePath((0, node_path_1.relative)(realOutputFolder, realFileDir))) {
        throw new Error(`Output file name escapes the output folder: ${pngPageOutput.name}`);
    }
    ...
    const realOutputFolderFinal = await node_fs_1.promises.realpath(resolvedOutputFolder);
